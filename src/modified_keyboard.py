...
from keyboard._canonical_names import all_modifiers, sided_modifiers, normalize_name
...
try:
    # Python2
    long, basestring
    def _is_str(x): return isinstance(x, basestring)
    def _is_number(x): return isinstance(x, (int, long))
...
def add_word_listener(word, callback, triggers=['space'], match_suffix=False, timeout=2):
    ...
    def handler(event):
        ...
        if name in triggers and matched:
            callback()
            state.current = ''
        elif name == 'backspace' and len(state.current >= 1):
